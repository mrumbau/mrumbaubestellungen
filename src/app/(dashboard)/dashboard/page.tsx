  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <StatCard label="Abweichungen" value={abweichungen} color="#dc2626" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-6">
        <div className="card p-5 border-l-[3px] border-l-[#dc2626]">
          <h2 className="font-headline text-sm text-[#1a1a1a] tracking-tight mb-4">Aktion erforderlich</h2>
          {aktionenNoetig.length === 0 ? (
            ...
            <div className="space-y-1 max-h-64 overflow-y-auto">
              {aktionenNoetig.slice(0, 10).map((b) => {
                ...
                return (
                  <Link key={b.id} href={`/bestellungen/${b.id}`} className="flex items-center justify-between p-3 rounded-lg hover:bg-[#fafaf9] transition-all group">
...
function StatCard({ label, value, color }: { label: string; value: number; color: string }) {
  return (
    <div className="card p-5" style={{ borderTop: `3px solid ${color}` }}>
      ...
      <p className="font-mono-amount text-3xl font-bold text-[#1a1a1a] mt-2">{value}</p>
    </div>
  );
}
